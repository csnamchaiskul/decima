...
export default function CrmUserTable() {
  ...
  const columns = [
    {
      ...
      render: (email, record) => (
        <span>
          {record.id === actorId && <Icon type={"user"} />}
          {email}
        </span>
      )
    },
    {
      ...
      render: (role, record) => {
        return (
          <Button
            onClick={() => {
              dispatch(adminActions.toggleAdminCrmUser());
            }}
            type={"link"}
          >
            {checked(!!role.find(r => r === "ROLE_CRMADMIN"))}
          </Button>
        );
      }
    },
    {
      ...
      render: (locked, record) => {
        return (
          <Button
            onClick={() => {
              dispatch(adminActions.toggleLockCrmUser());
            }}
            type={"link"}
          >
  ...
  const crmUserListLastUpdate = useSelector(
    adminActions.selector("crmUserListLastUpdate")
  );
